...
void Grafo::algoritmoPrim()
{
    ...
    cout<<endl<<endl<<"Peso Total: "<<pesoTotal<<endl<<endl;
    file<<endl<<endl<<"Peso Total: "<<pesoTotal<<endl<<endl;
...
void Grafo::buscaEmLargura(int id)
{
    ...
    if(!existe)
    {
        cout<<"VERTICE NAO ESTA DENTRO DO GRAFO"<<endl;
        file<<"VERTICE NAO ESTA DENTRO DO GRAFO"<<endl;
        exit(0);
    }
    ...
    while(!fila.empty())
    {
        ...
        file<<"Vertice "<< listaAdj[indiceNo].getId() <<" visitado"<<endl;
        fila.pop();
        for(std::vector<Aresta>::iterator arest = listaAdj[indiceNo].listaAresta.begin(); arest != listaAdj[indiceNo].listaAresta.end(); ++arest)
        {
            ...
            {
                listaAdj[arest->getIndiceNo()].setVisitado(true);
...
void Grafo::compConexa()
{
    ...
    else
    {
        ...
        for(int i=0; i < listaAdj.size(); i++)
        {
            if(visitado[i] == false)
            {
                cout<<endl<<endl<<"Componente Conexa -> ";
...
void Grafo::fortConexaUtil(int u, int dem[], int low[], stack<int> *membro, bool *verificaMembro)
{
    ...
    if(low[u] == dem[u])
    {
        ...
        {
            aux = (int) membro->top();
            cout << listaAdj[aux].getId() << " ";
            file << listaAdj[aux].getId() << " ";
...
void Grafo::ordenacaoTopologica()
{
    ...
    if(temCiclo()==true)
    {
        ...
        file << "Nao eh possivel fazer uma ordenacao topologica pois o grafo possui pelo menos um ciclo";
...
void Grafo::dijkstra(int id)
{
    ...
    //se o vertice nao exite no grafo o programa encerra
    if(!existe)
    {
        cout<<"VERTICE NAO ESTA NO GRAFO"<<endl;
        file<<"VERTICE NAO ESTA NO GRAFO"<<endl;
        exit(0);
    }
    ...
    int menor=999999;
    int k=0;
    //verifica se todos nos foram visitados
    while(!verificaVetorVazioDijkstra(vetorindice,listaAdj.size())&&k<listaAdj.size())
    {
        ...
        //percorre o grafo e verifica as condicoes para atribuir as distancias
        for(std::vector<Aresta>::iterator arest = listaAdj[indiceNoOrigem].listaAresta.begin(); arest != listaAdj[indiceNoOrigem].listaAresta.end(); ++arest)
        {
            ...
            if(arest->getPesoAresta()<0)
            {
                ...
                file<<"DIJKSTRA NAO ACEITA ARESTAS COM VALORES NEGATIVOS"<<endl;
                exit(0);
            }
    ...
    for(int i=0; i<listaAdj.size(); i++)
    {
        ...
        file<<listaAdj[i].getId()<<"\t \t"<<distancia[i]<<endl;
    }
